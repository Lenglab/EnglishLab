...
function showGrade(grade) {
  ...
  document.getElementById(
    "grade-title"
  ).textContent = `Temas de grado ${capitalize(grade)}`;
...
function showPeriod(period) {
  ...
  document
    .querySelector(`.period[onclick="showPeriod('${period}')"]`)
  ...
  if (
    topicsByGrade[currentGrade] &&
    topicsByGrade[currentGrade][`periodo${period}`]
  ) {
      ...
      if (periodo.hasOwnProperty(key)) {
        ...
        button.addEventListener("click", () => {
          ...
          localStorage.setItem(
            "selectedTopic",
            JSON.stringify(selectedTopicData)
          );
